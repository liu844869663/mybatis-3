...
public class PooledDataSource implements DataSource {
	...
	private PooledConnection popConnection(String username, String password) throws SQLException {
		...
		while (conn == null) {
			synchronized (state) { // 对整个连接池对象进行添加锁，保证并发情况下不出现问题，颗粒度很大，非常影响性能
				if (!state.idleConnections.isEmpty()) { // 空闲连接非空
					...
					if (state.activeConnections.size() < poolMaximumActiveConnections) { // 活动的连接数小于 poolMaximumActiveConnections
					...
					} else { // 既没有空闲连接，活动的连接已达到最大
						// Cannot create new connection
						// 获得首个激活的 PooledConnection 对象
						PooledConnection oldestActiveConnection = state.activeConnections.get(0);
						// 检查该连接是否超时
						long longestCheckoutTime = oldestActiveConnection.getCheckoutTime();
						if (longestCheckoutTime > poolMaximumCheckoutTime) { // 超时了
